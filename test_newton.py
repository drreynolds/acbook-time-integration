...
def f(x):           # nonlinear residual
    ff = np.array([x[0] + 0.004*x[0] - 1e3*x[1]*x[2] - 1.0,
...
for i in range(ntols):
    ...
    for lag in lags:
        sol = newton(f, Jsolver_dense, x0, rtol=rtols[i], atol=atols[i], Jfreq=lag)
        print("  dense,  Jfreq=", lag, ": success =", sol['success'], ", iters =", sol['iters'])

        sol = newton(f, Jsolver_sparse, x0, rtol=rtols[i], atol=atols[i], Jfreq=lag)
        print("  sparse, Jfreq=", lag, ": success =", sol['success'], ", iters =", sol['iters'])
