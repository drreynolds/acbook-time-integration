...
def ytrue(tt):
    ...
    """
    return np.array([np.sqrt(3.0 + np.cos(omega * tt)), np.sqrt(2.0 + np.cos(tt))], dtype=float)
...
def f(tt, yy):
    ...
    """
    AB = np.array([(-3.0 + yy[0]**2 - np.cos(omega * tt)) / (2.0 * yy[0]),
                   (-2.0 + yy[1]**2 - np.cos(tt)) / (2.0 * yy[1])], dtype=float)
    Udot = C@AB - np.array([omega*np.sin(omega * tt) / (2.0 * yy[0]), np.sin(tt) / (2.0 * yy[1])], dtype=float)
    return Udot
...
def J_dense(tt, yy):
    ...
    """
    dA_dU1 = 1.0 - (-3.0 + yy[0]**2 - np.cos(omega * tt)) / (2 * yy[0]**2)
    dB_dU2 = 1.0 - (-2.0 + yy[1]**2 - np.cos(tt)) / (2.0 * yy[1]**2)
    return np.array([[lambda_f * dA_dU1 + omega * np.sin(omega*tt) / (2.0 * yy[0]**2),
                      (1.0 - epsilon) * (lambda_f - lambda_s) / alpha * dB_dU2],
                     [-alpha * epsilon * (lambda_f - lambda_s) * dA_dU1,
                      lambda_s * dB_dU2 + np.sin(tt) / (2.0 * yy[1]**2)]], dtype=float)
...
def J_sparse(tt, yy):
    ...
    """
    return csc_array(J_dense(tt, yy))


def J_matvec(tt, yy, v):
    """
    Jacobian-vector product function, J@v
    """
    return J_dense(tt, yy)@v
...
if __name__ == "__main__":
    ...
    if test_dirk:
        ...
        A = np.array([[4024571134387.0/14474071345096, 0, 0, 0, 0],
                      ...
                      [2144716224527.0/9320917548702, -397905335951.0/4008788611757, 4024571134387.0/14474071345096,
                       0, 0],
                      [-291541413000.0/6267936762551, 226761949132.0/4473940808273, -1282248297070.0/9697416712681,
                       4024571134387.0/14474071345096, 0],
                      [-2481679516057.0/4626464057815, -197112422687.0/6604378783090, 3952887910906.0/9713059315593,
                       4906835613583.0/8134926921134, 4024571134387.0/14474071345096]], dtype=float)
        b = np.array([-2522702558582.0/12162329469185, 1018267903655.0/12907234417901, 4542392826351.0/13702606430957,
                      5001116467727.0/12224457745473, 1509636094297.0/3891594770934], dtype=float)
        c = np.array([4024571134387.0/14474071345096, 5555633399575.0/5431021154178, 5255299487392.0/12852514622453,
                      3.0/20, 10449500210709.0/14474071345096], dtype=float)
        hvals = (tf-t0)/Nout/np.array([4, 8, 16, 32, 64, 128], dtype=float)
        if test_dense:
